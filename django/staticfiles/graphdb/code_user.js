...
Promise.all([
  ...
  .then(function(dataArray) {
    ...
    var cy = window.cy = cytoscape({
      ...
      // zoom: 1.5, // Set the initial zoom factor; 1 is the normal zoom level, 2 is double size, so on.
      // maxZoom: 10, // Set the maximum allowed zoom factor
    });
    var params = {
      name: 'euler',
      randomize: false,
  ...
  setTimeout(() => {
      $('#slider-edgeLengthVal').on('input', function() {
          var value = Number($(this).val())/this.max*100;
