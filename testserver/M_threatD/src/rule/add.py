...
class Add(Neo4jHandler):
    ...
    def get_log_properties(self):
        ...
        RETURN COLLECT(DISTINCT(keys)) as props_list
        """
        result = self.run(database=self.user_db, query=cypher)
        return result['props_list']

    # Add Rule Action
    def add_rule(self):
        __new_request = self.request
        ruleClass = __new_request.pop('ruleClass')
        if ruleClass == 'static':
            __new_request.pop('count')
            return self.add_static_rule(__new_request)
        if ruleClass == 'dynamic':
            return self.add_dynamic_rule(__new_request)

    # Static Rules
    def add_static_rule(self, request):
    ...
    def static_cypher(self, rule, logType):
        rule_properties = ""
        where_cypher = ""
        is_errorCode = 0
        ...
        if 'ruleCount' in rule:
            cypher += self.static_count_cypher(where_cypher, rule, is_errorCode)
            query_cypher += self.static_count_cypher(where_cypher, rule, is_errorCode)
        else:
            cypher += f"""
            ...
            MERGE (log)-[:DETECTED {{alert:0, sent:0}}]->(rule)
            """
        result = self.rule_merge_test(cypher, query_cypher, rule, logType, 'static')
        return result

    def static_count_cypher(self, where_cypher, rule, is_errorCode):
        count_cypher = f"""
    ...
    # Dynamic Rules
    def get_flow_check(self):
        flows = QueryDict(self.request['rules']).dict()
        count = int(self.request['rule_count'])
        flow_names = []
                        ...
                        if not value:
                            ...
                            if f'flow_og_{i}_1' in flows:
                                if flows[f'flow_og_{i}_1'] == value:
                                    ...
                                    continue
                            cypher = f"MATCH (flow:Flow {{flowName: '{value}'}}) RETURN COUNT(flow) AS count"
                            result = self.run(database=self.user_db, query=cypher)
                            if result['count'] > 0 :
        ...
        return self.get_flow_slot()

    def get_flow_slot(self):
        flows = QueryDict(self.request['rules']).dict()
        count = int(self.request['rule_count'])
        rules = {}
        for i in range(1, count+1):
    ...
    def dynamic_cypher(self, flows, wheres, rule, count, logType):
        ...
        for i, flow in flows.items():
            if 'count' in flow:
                count_cypher, with_cypher = self.dynamic_count_cypher(flow, i, rule, with_cypher)
    ...
    def dynamic_count_cypher(self, flow, i, rule, with_cypher):
        count_where_cypher = ''
        is_errorCode = ''
        for item in range(len(flow['key'])):
            if flow['key'][item] == 'errorCode':
                is_errorCode = 1
            count_where_cypher += f""" 
            firstLog.{flow['key'][item]}= flow{i}.{flow['key'][item]}
        ...
        CALL apoc.do.when(
            ...
            ' 
                WITH d_u_c_logs, c_logs[0..index] AS i_logs,c_firstLog, c_logs
                UNWIND i_logs AS i_log
                WITH apoc.coll.containqsAll(i_log, d_u_c_logs) AS contains, d_u_c_logs, c_logs, c_firstLog, i_log
                WHERE contains = true
                WITH COLLECT(i_log)[0] AS i_log, c_firstLog, c_logs
                WITH apoc.coll.indexOf(c_logs, i_log) AS index, c_logs, c_firstLog
                WITH c_logs[index] AS d_u_c_logs, c_firstLog[index] AS i_c_firstLog
